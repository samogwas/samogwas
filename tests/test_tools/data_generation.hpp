...
static NovoMatrixPtr NovoTranspose( const NovoMatrix& mat ) {
  auto m = std::make_shared<NovoMatrix>();
  m->reserve(mat.at(0)->size());
  for ( size_t r = 0; r < mat[0]->size(); ++ r ) {
    m->push_back( std::make_shared<Vec>(mat.size()) );
    for ( size_t c = 0; c < mat.size(); ++c ) {
      m->at(r)->at(c) = mat[c]->at(r);
    }
  }
...
NovoMatrixPtr GenerateClusteredData::gen() {
  auto mat = (*this)();
  auto matrix = std::make_shared<NovoMatrix>();
  for (int i = 0; i < mat->size(); ++i) {
    matrix->push_back( std::make_shared<Vec>(mat->at(i)) );
  }
  return matrix;
}
