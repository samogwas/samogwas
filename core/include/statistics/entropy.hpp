...
#include "utils/type_utils.hpp" // samogwas::Int2Type
...
struct Entropy
{
  ...
  double operator()(VIterator xBegin, VIterator xEnd, bool has_missing = false) {
    return compute(xBegin, xEnd, samogwas::Int2Type<EstimationMethodType>());
  }

  template<typename VecType>
  double operator()(const VecType& xVec, bool has_missing = false) {
    return compute(xVec.begin(), xVec.end(), samogwas::Int2Type<EstimationMethodType>());
  }
 ...
 protected:
  template<typename VIterator>
  double compute(VIterator xBegin, VIterator xEnd, samogwas::Int2Type<EMP>, bool has_missing = false);

  
  template<typename VIterator>
  double compute(VIterator xBegin, VIterator xEnd, samogwas::Int2Type<EXACT>, bool has_missing = false);
...
struct JointEntropy
{
  ...
  { return compute(xBegin, xEnd, yBegin, samogwas::Int2Type<EstimationMethodType>(), has_missing); }
  ...
  { return compute(xVec.begin(), xVec.end(), yVec.begin(), samogwas::Int2Type<EstimationMethodType>(), has_missing); }
  ...
  {
    return compute(xVec.begin(), xVec.end(), yVec.begin(), jointVec, samogwas::Int2Type<EstimationMethodType>());
  }
 ...
 protected:
  ...
  double compute(VIterator xBegin, VIterator xEnd, VIterator yBegin,
                 samogwas::Int2Type<EMP>, bool has_missing = false);

 
  template<typename VecType>
  double compute(const VecType& xVec, const VecType& yVec,  samogwas::Int2Type<PRO_BT>);
...
template<int EstimationMethodType>
template<typename VIterator>
double Entropy<EstimationMethodType>::compute(VIterator xBegin, VIterator xEnd, samogwas::Int2Type<EMP>, bool has_missing )
{
