...
struct TestFisher { 
  ...
  double fisherTest( const ContigencyTabT& contigencyTab ) const {
    ...
    if ( nrow == 0 ) 
      return -9;
    int ncol = contigencyTab.empty() ? 0 : contigencyTab.size();
    if ( ncol == 0 ) 
      return -9;
    double table[200];
...
/*  fisher2.c: this is fexact.c, part of the R 
    ...
    (package ctest).
    Minor edits by az-Uriarte (rdiaz@cnio.es). May 2003.
*/
...
static void prterr(int icode, std::string mes);
...
#define rwrk ((float *)equiv)
    ...
    ikh = std::max(k * 5 + (kk << 1), nco * 7 + 800);
    iiwk= iwork(iwkmax, &iwkpt, ikh, i_int);
    ikh = std::max(nco + 401, k);
...
/*
  ...
  -----------------------------------------------------------------------
*/
inline void f2xact(int *nrow, int *ncol, double *table, int *ldtabl,
...
#else
                d1 = std::max(0., tmp + (pastp + drn) * 2.) / 2.;
		        d2 = df / 2.;
...
inline void f3xact(int *nrow, int *irow, int *ncol, int *icol, double *dlp,
...
inline void f4xact(int *nrow, int *irow, int *ncol, int *icol, double *dsp,
...
inline void f5xact(double *pastp, const double *tol, int *kval, int *key, int *ldkey,
...
/*
  ...
  -----------------------------------------------------------------------
*/
inline void f6xact(int *nrow, int *irow, int *iflag, int *kyy, int *key, int
...
inline void f7xact(int *nrow, int *imax, int *idif, int *k, int *ks,
       int *iflag)
...
inline void f8xact(int *irow, int *is, int *i1, int *izero, int *knew)
{
    int i;
...
inline double f9xact(int *n, int *mm, int *ir, double *fact)
{
    double d;
    int k;
...
inline void f10act(int *nrow, int *irow, int *ncol, int *icol, double *val,
...
/*
  ...
  -----------------------------------------------------------------------
*/
inline void f11act(int *irow, int *i1, int *i2, int *knew)
{
    int i;
...
inline void prterr(int icode, std::string mes) { error("FEXACT error " + int2str(icode)); }
...
/*
  ...
  -----------------------------------------------------------------------
*/
inline int iwork(int iwkmax, int *iwkpt, int number, int itype)
{
    int i;
...
inline void isort(int *n, int *ix)
{
  /*
  -----------------------------------------------------------------------
  Name:      ISORT
...
inline double gammds(double *y, double *p, int *ifault)
{
  /*
  -----------------------------------------------------------------------
  Name:      GAMMDS
...
inline double alogam(double *x, int *ifault)
{
  /* Initialized data */
