 ...
 ***************************************************************************************/
#ifndef FLTM_GWAS_OPTIONS_HPP
#define FLTM_GWAS_OPTIONS_HPP
...
inline Options get_program_options(int argc, char** argv) {
  ...
  try  {
    ...
    optDesc.add_options()
        ...
        ("thresholdFile,f", po::value<std::string>(&result.thresholdFile)->default_value("../example/inputs/thresholds.txt"), "Thresholds. Default: ../example/inputs/thresholds.csv")
        ("test,t", po::value<int>(&result.stat_test)->default_value(0), "test: 0 (chi-square)")
        ("sep,s", po::value<char>(&result.separator)->default_value('\t'), "Output file delimiter. Default: 'tab'")
